...
측정된 고정 사실은 하나다: 교차 검증 테스트 1개를 작성할 때 Arrange가 패턴 A는 3줄·협력자 2개, 패턴 B는 1줄·협력자 1개다(테스트 1개당 2줄, 협력자 1개 차이).

이 차이는 **능력의 차이가 아니라 기본값(default)의 차이다.** 패턴 B는 ViewModel이 값을 스냅샷으로만 볼 수 있는 구조라 검증이 순수 함수로 **강제된다.** 패턴 A는 ViewModel이 `TextFieldState`를 쥐고 있어서 "검증도 거기서 읽는" 쪽이 손이 덜 가고, 그 결과 테스트가 ViewModel 인스턴스화와 상태 주입을 요구한다. 패턴 A에서도 "검증 로직은 항상 순수 함수로 분리한다"는 규율을 지키면 이 차이는 사라진다.
...
| 패턴 A+Saved | **복원** — 5개 필드 모두 입력값 유지 | 1회 고정 8줄(헬퍼) + import 4개 + 필드당 1줄 | `s_error` "종료일이 시작일보다 빠릅니다" 유지 |
| 패턴 B | **복원** — 5개 필드 모두 입력값 유지 | 저장/복원 로직 **0줄** (필드 선언 `rememberTextFieldState()`는 두 패턴 모두 필요) | `b_error` "종료일이 시작일보다 빠릅니다" 유지 |

패턴 A+Saved가 `FormViewModelA`에 추가한 코드의 스케일링 특성: **최초 1회 고정 비용**(`restoringField` 헬퍼 약 8줄 + import 4개)과 **필드당 한계 비용 1줄**(`val title = handle.restoringField("title")`)로 나뉜다. 즉 필드가 20개인 폼이면 대략 8줄(고정) + 20줄(필드당)이지 "필드마다 8줄"이 아니다. 패턴 B의 "0줄"도 저장/복원 로직에 한정된 것이고, 필드 선언(`rememberTextFieldState()` 호출) 자체는 패턴 A와 마찬가지로 필드 수만큼 있어야 한다.
...
프로세스 종료 복원이 요구사항이면 **패턴 B가 기본값으로 복원을 제공한다** — `rememberTextFieldState()`가 저장/복원 로직 0줄로 처리한다. 패턴 A는 `SavedStateHandle` 연동 보일러플레이트(1회 고정 8줄 + 필드당 1줄)를 직접 작성해야 하고, 이를 빠뜨리면 조용히 소실된다. 보일러플레이트가 필드 수에 비례해 폭증하는 것은 아니지만, "안 쓰면 소실"이라는 기본 동작의 방향이 두 패턴에서 반대다.
...
아래 권장은 "패턴 B가 우수하다"는 뜻이 아니라, **패턴 B는 해당 이점을 기본값으로 강제하고 패턴 A는 규율 또는 1회성 보일러플레이트로 동급이 된다**는 뜻이다.

| 상황 | 권장 패턴 | 근거 |
|---|---|---|
| 교차 필드 검증 없음 + 복원 요구 없음 | 아무거나 (패턴 A가 더 단순) | 세 실험 모두 유의미한 차이 없음. config change는 두 패턴 모두 통과. |
| 교차 필드 검증 있음 | **패턴 B**, 또는 "검증은 순수 함수로 분리" 규율을 지키는 **패턴 A** | 패턴 B는 값 스냅샷 구조라 순수 함수 검증이 강제되어 테스트 Arrange가 1줄·협력자 1개. 패턴 A는 그 규율을 팀이 지키면 동일해지고, 안 지키면 테스트 1개당 2줄·협력자 1개가 더 든다. 능력 차이가 아니라 기본값 차이다. |
| 프로세스 종료 복원 요구 있음 | **패턴 B**, 또는 `SavedStateHandle` 보일러플레이트를 감수하는 **패턴 A+Saved** | 패턴 B는 저장/복원 로직 0줄. 패턴 A+Saved는 1회 고정 8줄 + 필드당 1줄이며 필드 수에 비례해 폭증하지는 않는다. 결정적 차이는 분량이 아니라 "코드를 안 쓰면 소실"이라는 기본 동작의 방향이다. |
| 위 두 요구가 모두 있음 | **패턴 B** (또는 두 규율·보일러플레이트를 모두 감수하는 패턴 A) | 패턴 B는 두 이점이 추가 비용 없이 겹친다. |
...
한 줄 요약: **교차 검증이 있거나 프로세스 종료 복원이 필요하면 패턴 B가 그 이점을 기본값으로 준다. 패턴 A도 규율(순수 함수 분리)과 1회성 보일러플레이트(`SavedStateHandle` 헬퍼)를 추가하면 동급이 되며, 두 요구가 모두 없으면 패턴 A로 충분하다.**
